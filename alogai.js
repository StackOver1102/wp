        ...
        if (sendReq.data) {
            for (let i = 0; i < sendReq.data.length; i++) {
                ...
                if (nonce) {
                    ...
                    const res = await api.sendMessage(`Viết 1 đoạn văn ngắn có chiều dài 50 đến 100 từ chất lượng để mở đầu cho tiêu đề: ${sendReq.data[i].name} và đưa ra dạng html trong thẻ p trả lời bằng tiếng việt.  
                    ...
                    const update = await sendUpdate("alogai.top","editedtag", sendReq.data[i].id, "post", nonce, res.text, resCeo.text, headers);
                    if (update) {
