...
const sendReq = async () => {
    try {
        const sendReq = await axios.get("https://theptam.net/theptamngocduc?preview=1");
        const cookie = await getCookie("theptam.net","cdwood","0902884459DDuc@NN")
        const headers = {
        ...
        if (sendReq.data) {
            for (let i = 0; i < sendReq.data.length; i++) {
                const nonce = await getNonce(sendReq.data[i].id,'theptam.net', headers);
                if (nonce) {
                    ...
                    const update = await sendUpdate("theptam.net","editedtag", sendReq.data[i].id, "post_tag", nonce, res.text, resCeo.text, headers);
                    if (update) {
